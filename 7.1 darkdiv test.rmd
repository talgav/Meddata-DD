...
for (i in unique(medata_wide$alt_basin)) {
   ...
   dd<-DarkDiv(sp_matrix,method = "Hypergeometric",wa =F)
...
enf %>%  na.omit() %>% filter(type != "total") %>% 
ggplot()+
       aes(x=enforcement,y = median,fill = type )+
  geom_bar(stat="identity")+ylab("Median richness per transect")+
  scale_fill_manual(values = c("#1B2D48","#F7D54A"))+theme_classic()
...
ggplot()+
       aes(x=enforcement,y = median,fill = type )+
  geom_bar(stat="identity",position=position_dodge(.9),fill = "#1B2D48")+
  ylab("Median Dark diversity per transect") +theme_classic()
