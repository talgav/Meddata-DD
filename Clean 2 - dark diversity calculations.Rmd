---
title: "Clean 2 - dark diversity calculations"
author: "TG"
date: "2024-01-11"
output:
...
load("G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_Medata_lis.Rdata")
...
* `Clean_Medata.Rdata` - the clean MeData after all the alternations *or*

* `Clean_Medata_lis.Rdata` - the clean MeData after all the alternations with Lisandro data
...
for (i in unique(medata_wide$Basin)) {
   ...
   sp_matrix<-sp_matrix[,colSums(sp_matrix != 0) > 0] # Remove species with that have no ocurrance in the basin
   ...
   dd_dark<-as.data.frame(dd_dark) # convert from matrix to dataframe
...
1. Removal of transect with less than two observed species 
2. Removal of zero enforcement transects as they extremely variable and have large differences in  the actual fishing pressure

### Save dark diversity MeData 

```{r}
# save(dd_data, file = "G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_MeData_dd_lis.Rdata")

# write.csv(dd_data,"G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_MeData_dd_lis.csv")
```

# Complexity dark diversity

```{r}
load("G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_complexity_data.Rdata")
```
...
c_dd_dark<-as.data.frame(c_dd_dark) # convert to dataframe
...
```{r}
# save(comp_dd, file = "G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_complexity_dd.Rdata")

# write.csv(comp_dd,"G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\clean output data\\Clean_complexity_dd.csv")
