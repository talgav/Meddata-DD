...
```{r locations map}
location_sites <- cbind(locations, sites) %>% 
  na.omit(.)

ggplot(location_sites, aes(x = lon, y = lat)) + geom_point()
...
'raw_med' is an extensive dataset which includes all data collected, in a long format (so each species in each site has its own row). Though it is called "raw data", it had actually already undergo a series of validations (which are not done yet!).
