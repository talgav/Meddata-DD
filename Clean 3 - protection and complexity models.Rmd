...
protction_model_data<-dd_data %>% filter(observed_richness >1)

# remove 0 enforcement

protction_model_data<-protction_model_data %>% filter(enforcement != 0)

# drop level of the site column
...
#tab_model(Total, Observed, Dark, dv.labels = c("Species pool", "Observed richness", "Dark diversity"), file = "protection_model_results.xls")
...
completeness_model <- glmmTMB(completeness ~ enforcement + (1|Basin:Site),
...
pred_completeness=ggpredict(completeness_model,terms = c('enforcement[1:3 by=1]'))
...
plot_completness_pro <-ggplot()+
  geom_line(data = pred_completeness,aes(x= x,y = predicted),size=1.5,color= "red")+
  geom_ribbon(data = pred_completeness,aes(x=x,ymin=conf.low,ymax=conf.high),alpha=0.2,fill="red")+
  xlab("Protection") + 
  scale_x_continuous(limits=c(1,3),breaks = c(1,2,3)) +
  ggtitle(label = "Completeness",
          subtitle = subtitle)+
  ylab("Completeness")+theme_classic()


plot_completness_pro
rm(p,subtitle,pred_completeness)
...
```{r}
#tab_model(completeness_model,file = "Completeness_protection_model.xls")
...
ggsave("predictions_plot.pdf",predictions_plot,path = dir, width = 10, height = 10)
...
# tab_model(completeness_model, completness_curve, completeness_vr, dv.labels = c("Protection", "Curvature", "Vertical relief"), file = "completenes_model_results.xls")
