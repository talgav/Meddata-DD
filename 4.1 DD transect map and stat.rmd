...
basin <- basin %>%  select(Basin,unique_trans_id) %>% distinct(unique_trans_id,.keep_all = T)
...
```{r}
unique_coords<-medata %>% distinct(lat,lon,enforcement,protection,.keep_all = T) %>% select(country,site,lat,lon,protection,enforcement)
