...
```{r setup data}
med_raw <- read_csv("data/all_uvc_data_101119.csv", na = c("", "NA"),
...
Data was collected at 65 sites:
```{r}
med_raw %>% distinct(site) %>% arrange(.$site) %>% print(n = Inf) # full site list

```

## Species

Dataset withholds 122 species (NOTE: some are not species but genus or family.)
```{r}
med_raw %>% distinct(species) %>% arrange(.$species) %>% print(n = Inf) # full species list. 
```

# Methods

## Data collection
Visual fish census surveys took place in multiple locations along the Mediterranean Sea (figure 1) by teams of skilled SCUBA divers. Locations were comprised of sites within MPAs with varying size, age and enforcement level, and unprotected sites which are adjacent to these MPAs. To date, the database is comprised of approximately 50,0000 observations of 93 species of fish and includes abundance data, as well as presence-absence data. Additional data will be collected during 2019 and will be added to the main database.
...
Fish **traits** data ("fish_traits.csv" including: size, mobility, activity, schooling, position, diet) were aqcuired from [GASPAR](http://cesab.org/index.php/en/projets-passes/28-gaspar). Where data were unavailable we used the information from [Trait structure reveals the processes underlying fish establishment in the Mediterranean. O Givan, V Parravicini, M Kulbicki, J Belmaker - Global ecology and biogeography, 2017](https://onlinelibrary.wiley.com/doi/full/10.1111/geb.12523).
...
* Data from source `azz_asi` are only presence-absence.
