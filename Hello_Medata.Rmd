...
countries <- med_raw %>% distinct(country) %>% arrange(.$country) %>% print(n = Inf)# full country list
sites <- med_raw %>% distinct(site) %>% arrange(.$site) %>% print(n = Inf) # full site list
species <- med_raw %>% distinct(species) %>% arrange(.$species) %>% print(n = Inf) # full species list. NOTE: some are not species but genus or family.
