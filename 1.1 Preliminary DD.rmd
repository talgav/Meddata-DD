...
plot_dir<-"G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\figures\\Initial DD plots"
```
...
beals[,20:142] <- beals(beals[,20:142],type = 0,include = FALSE) 
...
By country and protection:

```{r}
country<-dark_diversity %>%
  group_by(country,protection,dark_sp,unique_id) %>%
  summarise(richness = n_distinct(species)) %>% arrange(unique_id)


country<-spread(country,dark_sp,richness,fill = 0)

country$total<-country$dark + country$observed

country <- gather(country,type,richness,4:6)
country <- country %>% arrange(unique_id)

country <- country %>%
  group_by(country,protection,type) %>%
  summarise("mean_richness"= mean(richness),
            "se"=std.error(richness)) %>%  na.omit()
...
ggplot()+
       aes(x=protection,y = mean_richness,fill = type )+
  geom_bar(stat="identity",position=position_dodge(.9))+
  geom_errorbar(aes(ymin=mean_richness-se, ymax=mean_richness+se), width=.2,position=position_dodge(.9))+ylab("Mean richness per transect") 
```
by temp

```{r}
temp<-dark_diversity %>%
  group_by(tmean,protection,dark_sp,unique_id) %>%
  summarise(richness = n_distinct(species)) %>% arrange(unique_id)


temp<-spread(temp,dark_sp,richness,fill = 0)

temp$total<-temp$dark + temp$observed

temp <- gather(temp,type,richness,4:6)
temp <- temp %>% arrange(unique_id) %>% na.omit()

temp %>% 
ggplot()+aes(x=tmean,y=richness,color = type)+
  geom_point()+geom_smooth(method = "lm")+
  facet_wrap(~protection)

ggsave(path = plot_dir, filename = "temo_mpa.png")

temp %>% filter(type == "dark") %>% 
ggplot()+aes(x=tmean,y=richness,color = protection)+
  geom_point()+geom_smooth(method = "lm")+
  ylab("Dark diversity")

#ggsave(path = plot_dir, filename = "dark_temp_mpa.png")
