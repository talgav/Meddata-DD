---
title: "Clean 4 - log ratios and pairwise comparisons"
author: "TG"
date: "2023-11-29"
output:
...
dir<-"G:\\My Drive\\PHD\\chapter 3 MPA dd\\Meddata-DD\\final plots and models\\log ratios and pairwise"
...
all plots saved in `log ratios and pairwise` folder

* `fig_4_left` - plot of log ratio + metafor models (Fig 4)

* `fig_4_right` - plot of pairwise comparison of the 3 study cases (Fig 4)

* `Completeness_effect_size` - plot of log ratio + metafor models for completeness (Appendix 6) 
...
use the `batch_calc_ES` function to calculate the log-ratio for the diversity component: dark diversity, observed richness and site species pool
...
protected_dark <-effect_sizes_dark %>% filter(pair %in% fully_protected_pairs$pair) 
...
protected_obs <- effect_sizes_obs %>% filter(pair %in% fully_protected_pairs$pair) 
...
protected_total <-effect_sizes_total %>% filter(pair %in% fully_protected_pairs$pair) 
...
Medes <- dd_data %>% filter(Site == "medes")
...
ggsave("fig_4_left.pdf",fig_4_left,path = dir, width = 5, height = 10)

fig_4_right <- Plot_banyuls/Plot_medes/Plot_brijoni 
fig_4_right

ggsave("fig_4_right.pdf",fig_4_right,path = dir, width = 5, height = 10)
...
protected_comp <-effect_sizes_comp %>% filter(pair %in% fully_protected_pairs$pair) 
...
#ggsave("Completeness_effect_size.pdf",path = dir,width = 5,height = 10)
